...
- **Full pipeline parity** — session, conversation, journal, AND weekly check all feed the identical mastery + SRS pipeline. **CAVEAT (2026-06-06):** the *AI-detected-error* writes from conversation and journal are now GATED OFF (`CONVERSATION_CORRECTIONS_ENABLED` / `JOURNAL_ERROR_WRITE_ENABLED` = false) — those surfaces still feed *deterministic* signal (production bricks, speaking minutes, constraint results) but no longer write unverified AI corrections to mastery. Re-enable per surface once a deterministic corrector exists.
