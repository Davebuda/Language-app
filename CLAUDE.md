...
- **Full pipeline parity** — session, conversation, journal, AND weekly check all feed the identical mastery + SRS pipeline. **CAVEAT (updated 2026-06-08, Lever 3 `467b252` deployed):** the old blanket gate (`CONVERSATION_CORRECTIONS_ENABLED` / `JOURNAL_ERROR_WRITE_ENABLED`) is replaced by a per-correction deterministic verifier. Conversation + journal now write AI-detected errors to mastery again — but ONLY lexicon-confirmed **noun-gender** corrections (`verifyGenderCorrection`); every other AI-claimed class (V2, conjugation, …) stays show-don't-grade. Deterministic signal (production bricks, speaking minutes, constraint results) feeds as before. Re-arm the remaining classes once each has its own deterministic check.
