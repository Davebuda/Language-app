export type LaneId = 'session' | 'journal' | 'conversation' | 'roleplay' | 'reading' | 'listen' | 'drills' | 'shadow' | 'uke' | 'ord'
...
function emptyLanes(): Record<LaneId, boolean> {
  return { session: false, journal: false, conversation: false, roleplay: false, reading: false, listen: false, drills: false, shadow: false, uke: false, ord: false }
}
