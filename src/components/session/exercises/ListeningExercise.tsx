...
export function ListeningExercise({ item, sentence, sessionId, onResult }: ListeningExerciseProps) {
  ...
  const audioButtonClass =
    'min-h-[48px] rounded-[1rem] border border-[rgba(4,14,8,0.12)] bg-white/45 px-5 py-3 text-sm font-semibold text-nc-cream-text transition-colors hover:bg-white/60 disabled:opacity-40'

  return (
    <div className="space-y-5">
      <p className="text-[12px] font-bold uppercase tracking-[0.08em] text-nc-cream-dim">
      ...
      {hasAudioFile ? (
        ...
        <div className="rounded-[1rem] border border-[rgba(4,14,8,0.12)] bg-white/45 px-5 py-4 text-sm text-nc-cream-muted">
      ...
      <button
        ...
        className="nc-button-primary min-h-[52px] w-full px-6 py-3 font-bold disabled:cursor-not-allowed disabled:opacity-30 disabled:shadow-none"
      >
        Sjekk svar
      </button>
