...
## 2026-05-21 RESTRUCTURE (out-of-band) — Pronunciation drills mode shipped

Commit fd9e5e7 built `/drills` outside of council scope. Assessment:
- 4 drill sets (kj, sj, ø, retrofleks), each with 5 target words ✅
- `DrillsScreen`: selection → drill → completion with pass/fail breakdown ✅
- `DrillExercise`: same idle→listen→result pattern as shadowing; heuristic hint layer ✅
- `pronunciationHeuristics.ts`: maps known English-speaker substitutions to Norwegian coaching tips ✅
- `speechMatchUtils.ts`: shared with shadowing — `computeMatchScore`, `computeWordMatches` extracted ✅
- `ShadowingExercise` updated to import from shared util (duplication removed) ✅
- Fingerprint integration via `recordResult` ✅

## 2026-05-21 CORRECT → APPROVE — Pronunciation Drills mode

**CORRECT issued for two blockers:**
1. `scheduler.ts:50 pickExerciseType` — dead code causing ESLint error that blocks `next build`. Function was replaced in a prior commit by the `addItem` closure. Fix: deleted the function (3 lines).
2. `/drills` and `/shadow` — orphaned pages with no dashboard navigation path. Fix: added "Skygging" and "Uttaleøvelser" secondary links to the MUNTLIG card on the dashboard.

**After corrections:**
- `next build` passes ✅
- Dashboard MUNTLIG card: Start samtale / Skygging / Uttaleøvelser all link correctly ✅
- `/drills` selection + exercise flow: Playwright PASS ✅
- 0 console errors on `/drills` ✅

Playwright: PASS — report at `.council/reports/2026-05-21-drills-mode-verification.md`
Screenshots: `drills-exercise-kjlyd.png`, `drills-dashboard-muntlig-links.png`
