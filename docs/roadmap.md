...
**Stream 5 close honesty note (added 2026-05-22T10:15, per Operating Rule 6):** Stream 5 was claimed closed 2026-05-22T04:20 with all 8 phases shipped. Stream 5.5 Phase 2 investigation surfaced that Phase 5b carried an unshipped TODO in production code — `closeWeek` in `src/engine/weekly-sprint.ts` hardcoded `focusOutcomes[conceptId].startScore = 0` with the comment "Phase 5 will capture startScore at week-open". The startScore-at-open snapshot was never implemented in Phase 5. The TODO was closed retroactively as part of Stream 5.5 Phase 2 (Phase A1) by adding `weekStartSnapshots` to the fingerprint, populating it in `openWeek`, and consuming + clearing it in `closeWeek`. Historical `weeklySprintHistory` records written before 2026-05-22T10:15 retain `startScore: 0` (forward-only fix; no retrofit). All weekly records written from this point forward carry the real captured startScore.

**Holistic Vision-vs-Shipped Audit (2026-05-22T15:00):** User direction triggered a system-level audit of how four production surfaces (journal, reading/lese, listening/lytte, muntlig) trace to the documented vision. Audit was orchestrated through `super-orchestrator` (background agent `ad56db38f724903d6`) and produced a single sequenced restructure plan. Headline findings: (1) `/listen` rendered questions as text not audio AND routed every recordResult through a hardcoded `'speaking-production'` catch-all conceptId — Operating Rule 6 + 8 violation. (2) `/roleplay` had the same conceptId catch-all on all 12 turns across 3 scenarios. (3) Reading is recognition-only — no production demand. (4) Journal is engine-fed but invisible to `/uke`. (5) C1 not feasible on free local AI per validation research (~50% edge-case failure). Architect-α ratification by user: ship the bug-only sliver (fix the `conceptId` lies in Listening + Roleplay) and **resume the ratified Stream 5.5 sequence**. Broader restructure (TTS, expansion, journal archive, reading comprehension wrap, mobile pass) deferred — re-opens only if Council ratifies them as in-scope when Stream 5.5 closes. α path shipped 2026-05-22: α.1 = listening conceptId routing (commit `784e12b`); α.2 = roleplay conceptId routing (commit `1260c22`); α.3 = doc-sync + C1 disposition (this commit). Stream 5.5 Phase 3 (Journal weekly-focus prompt bias) is the next active phase.
...
**Phase 5b — Graduation rule.** ✅ COMPLETE 2026-05-22T01:03 (commit `85504e4`). `WeeklySprintRecord.focusOutcomes` extended with `graduated: boolean`. `closeWeek` signature gained `graph` parameter; computes per-concept graduation via `isGraduated` predicate (mastery threshold AND minAttempts; low check score <50 demotes; null check doesn't punish). All callers threaded through. 7 new tests. **Carry-over (closed retroactively 2026-05-22T10:15 via Stream 5.5 Phase 2):** `closeWeek` shipped with `focusOutcomes[…].startScore = 0` hardcoded and a TODO comment. Phase 5 never delivered the startScore-at-open snapshot. Closed by Stream 5.5 Phase 2 adding `weekStartSnapshots`. See the Stream 5 close honesty note in Current Position.
...
**Phase 1 — Reading concept-tagging + exposure logging.** ✅ COMPLETE 2026-05-22T09:34 (commit `6624937`). 4 reading texts tagged with verified concept IDs against `content/concepts/{a1,a2}-graph.json`. `logConceptExposure` added to `src/lib/logEvents.ts` (fire-and-forget, auth-only, `event_type='concept_exposure'`, `correct_bool: true` semantically). `recordExposure(conceptIds)` added to `useFingerprint.ts` — increments `attemptCount` by 0.3 per unique concept, updates `lastAttemptAt`, does NOT touch `rawScore`/`confidenceScore`/`decayedScore`. Text-close handlers in `src/app/reading/page.tsx` wired to call `recordExposure(selectedText.conceptIds)`. 13 new tests (6 in `tests/lib/exposure.test.ts` + 7 in `tests/hooks/recordExposure.test.ts`). Test count 183 → 196. Playwright SMOKE PASS (`.council/reports/2026-05-22-0934-phase1-reading-exposure.md`).
...
- **Listening module — full restructure** — Stream 5.5 lane ratification deferred Listening until Stream 3 audio infra ships. The 2026-05-22 holistic audit (see Current Position) proposed bridging with Web Speech API TTS + 30-question expansion; **deferred** per architect-α ratification — only the pipeline-honesty conceptId routing fix shipped as α.1 (commit `784e12b`). The TTS-bridge / question-expansion / "Lytt og svar" UX redesign re-opens only when (a) Council ratifies TTS as audio-infra-equivalent, OR (b) Stream 3 chatterbox-tts-norwegian pipeline lands. Until then, `/listen` remains a 7-question text-rendered surface with honest concept routing.
...
- **C1 concept graph + corpus + model upgrade** — added 2026-05-22 per holistic-audit ratification. CEFRLevel type stops at B2 today; C1 was never built into the UI (no removal needed). Full C1 capability requires three dependencies: (a) authored CEFR-tagged C1 concept graph in `content/concepts/c1-graph.json`, (b) native-validated C1 corpus (idiomatic Norwegian, formal-register sentences, subjunctive equivalents, advanced subordinate clauses), and (c) either upgraded local AI model OR a per-user-funded paid API path (Speechace/Azure/OpenAI). Validation research caps current NB-Llama-3.2-1B at ~50% reliability on edge cases — well below the bar for C1 production. **Positioning honesty:** NorskCoach is an A1→B2 coach. C1 learners need a different product or a paid tier.
...
**Updated 2026-06-06 — live-bug + integrity + moat-visibility sweep deployed (see latest Completed-phases entry).** Earlier: 2026-05-26 Stream 5.5 (Lanes on a Bar) complete + Waves 0-3 shipped. See the Stream 5.5 section above for full phase-by-phase closure; `docs/vision-and-plan.md` for the forward-looking execution plan; `CLAUDE.md` Current State (2026-06-06 box) for the canonical live snapshot.
...
- **Live-bug + integrity + moat-visibility sweep (2026-06-05/06):** all deployed to pandoai.no (HEAD `da63374`, 685 tests, tsc clean). (a) server-side Groq generation LIVE (`0e573aa`) — mobile path no longer dead; (b) corpus integrity 112 audit errors → 0 (`ec41b09`, canonical-17 tag remap + difficulty clamp + dedup); (c) 39 B1 word-order keys repaired (`2c7337c`) — were slash-corrupted scrambles in `norwegian`, unwinnable → phantom errors; (d) repair-loop phantom-type filter (`930df13`); (e) **AI grammar corrections decoupled from the fingerprint** — conversation (`36f4b73`) + journal (`cf5141b`) — the Groq 8B produced wrong-but-valid corrections (`et jobb`) that poisoned diagnosis; **new rule: no unverified AI output moves mastery**; (f) **root-cause diagnosis surfaced as the dashboard coach line** (`1165498`, tightened `da63374`), Norwegian, Playwright-QA'd at 375px — the moat made visible. Also: B1 corpus deepened to ~51/concept; QA-gate-agent reliability findings (norwegian-linguist works inline; architect swallows output). Authority: memory `project_engine_moat_gaps` (2026-06-05/06).
